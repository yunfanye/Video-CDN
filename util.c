...
void print_packet(struct packet* packet){
	...
	if(packet->header==NULL){
		...
		printf("Header:\nID: %d, QR: %d, Opcode: %d, AA: %d, TC: %d, RD: %d, RA: %d, Z:%d, RCODE: %d, QDCOUNT: %d, ANCOUNT: %d, NSCOUNT: %d, ARCOUNT: %d\n",
			...
			packet->header->Z, packet->header->RCODE, ntohs(packet->header->QDCOUNT), 
	...
	if(packet->resource_record==NULL){
		...
		printf("Resource record:\nNAME: %s, TYPE: %d, CLASS: %d, TTL: %d, RDLENGTH: %d, RDATA: %d\n",
			...
			ntohs(packet->resource_record->TTL), ntohs(packet->resource_record->RDLENGTH), ntohs(packet->resource_record->RDATA));
	}
}
...
char* make_response_packet(char* input_buffer, char* dest_addr, int* response_length){
	...
	*(uint16_t*)(response+length+sizeof(uint16_t)*2) = 0;
	*(uint16_t*)(response+length+sizeof(uint16_t)*3) = htons(4);
	struct sockaddr_in temp_addr;
	inet_pton(AF_INET, dest_addr, &(temp_addr.sin_addr));
	*(uint32_t*)(response+length+sizeof(uint16_t)*4) = htonl(temp_addr.sin_addr.s_addr);
